...
export default function CaptionExamplesPage() {
  ...
  async function load() {
    setLoading(true);
    const { data, error } = await supabase.from("caption_examples").select("*").order("id", { ascending: false }).limit(200);
    ...
    <div style={{ padding: "36px 40px" }}>
      ...
      {modal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "var(--bg-card)", borderRadius: 16, padding: 32, width: 520, maxWidth: "90vw", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 20px" }}>Edit Caption Example</h2>
            {Object.entries(modal.form).map(([k, v]) => (
              <div key={k} style={{ marginBottom: 14 }}>
                ...
                {String(k).toLowerCase().includes("caption") || String(k).toLowerCase().includes("content") ? (
                  <textarea
                    value={v === null || v === undefined ? "" : String(v)}
                    onChange={(e) => setModal((prev) => prev ? { ...prev, form: { ...prev.form, [k]: e.target.value } } : null)}
                    rows={4}
                    style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid var(--border)", fontSize: 13, outline: "none", boxSizing: "border-box", resize: "vertical", fontFamily: "inherit" }}
                  />
                ) : (
                  <input
                    value={v === null || v === undefined ? "" : String(v)}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 6px", letterSpacing: "-0.01em" }}>Caption Examples</h1>
          <p style={{ fontSize: 14, color: "var(--text-muted)", margin: 0 }}>{rows.length} example{rows.length !== 1 ? "s" : ""}</p>
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "var(--bg-subtle)", borderBottom: "1px solid var(--border-subtle)" }}>
                  {cols.map((c) => <th key={c} style={{ padding: "12px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "var(--text-muted)", letterSpacing: "0.06em", textTransform: "uppercase", whiteSpace: "nowrap" }}>{c}</th>)}
