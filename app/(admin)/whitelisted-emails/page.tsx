...
export default function WhitelistedEmailsPage() {
  ...
  function openCreate() {
    ...
    if (Object.keys(form).length === 0) { form.email = ""; }
    ...
    <div style={{ padding: "36px 40px" }}>
      ...
      {modal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "var(--bg-card)", borderRadius: 16, padding: 32, width: 440, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 20px" }}>
              {modal.mode === "create" ? "Add Whitelisted Email" : "Edit Email"}
            </h2>
            {Object.entries(modal.form).map(([k, v]) => (
              <div key={k} style={{ marginBottom: 14 }}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "var(--text-muted)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 5 }}>{k}</label>
                <input
                  type={k === "email" ? "email" : "text"}
                  value={v === null || v === undefined ? "" : String(v)}
                  onChange={(e) => setModal((prev) => prev ? { ...prev, form: { ...prev.form, [k]: e.target.value } } : null)}
                  placeholder={k === "email" ? "e.g. user@external.com" : ""}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={handleSave} disabled={saving} style={{ flex: 1, padding: "10px 0", background: "#111", color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.5 : 1 }}>
                {saving ? "Saving…" : modal.mode === "create" ? "Add Email" : "Save"}
              </button>
              <button onClick={() => setModal(null)} style={{ flex: 1, padding: "10px 0", background: "var(--bg-muted)", color: "var(--text-primary)", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 6px", letterSpacing: "-0.01em" }}>Whitelisted Emails</h1>
          <p style={{ fontSize: 14, color: "var(--text-muted)", margin: 0 }}>{rows.length} email{rows.length !== 1 ? "s" : ""}</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search…" style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid var(--border)", fontSize: 13, outline: "none", width: 200, background: "var(--bg-card)" }} />
          <button onClick={openCreate} style={{ padding: "8px 18px", borderRadius: 8, border: "none", background: "#111", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Add Email</button>
        </div>
      </div>
      ...
      <div style={{ background: "var(--bg-card)", borderRadius: 14, overflow: "hidden" }}>
        {loading ? (
          ...
          <div style={{ padding: 48, textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>No emails found.</div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "var(--bg-subtle)", borderBottom: "1px solid var(--border-subtle)" }}>
                {cols.map((c) => <th key={c} style={{ padding: "12px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "var(--text-muted)", letterSpacing: "0.06em", textTransform: "uppercase" }}>{c}</th>)}
            ...
            <tbody>
              {filtered.map((row, i) => (
                <tr key={i} style={{ borderBottom: i < filtered.length - 1 ? "1px solid var(--border-subtle)" : "none" }}>
                  {cols.map((c) => (
                    <td key={c} style={{ padding: "10px 16px", fontSize: 13, color: "var(--text-primary)" }}>
                      {typeof row[c] === "boolean" ? (
                        <span style={{ display: "inline-block", padding: "2px 10px", borderRadius: 999, fontSize: 11, fontWeight: 700, background: row[c] ? "#16a34a18" : "var(--bg-muted)", color: row[c] ? "#16a34a" : "var(--text-muted)" }}>{row[c] ? "Yes" : "No"}</span>
                      ) : formatVal(row[c])}
                    </td>
                  ))}
                  <td style={{ padding: "10px 16px" }}>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => openEdit(row)} style={{ padding: "6px 14px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-subtle)", color: "var(--text-primary)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Edit</button>
