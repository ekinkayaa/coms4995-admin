...
export default async function ReportedContentPage() {
  ...
  const card = { background: "var(--bg-card)", borderRadius: 14, overflow: "hidden" as const };
  const thStyle = { padding: "12px 16px", textAlign: "left" as const, fontSize: 11, fontWeight: 700, color: "var(--text-muted)", letterSpacing: "0.06em", textTransform: "uppercase" as const };
  const tdStyle = { padding: "12px 16px", fontSize: 13, color: "var(--text-primary)", verticalAlign: "top" as const };
  const emptyStyle = { padding: 40, textAlign: "center" as const, color: "var(--text-muted)", fontSize: 14 };
  ...
  return (
    <div style={{ padding: "36px 40px" }}>
      ...
      <h2 style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)", margin: "0 0 12px" }}>
        Reported Captions <span style={{ fontSize: 13, fontWeight: 400, color: "var(--text-muted)" }}>({reportedCaptions.length})</span>
      </h2>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {reportedCaptions.map((r: any, i: number) => (
                <tr key={r.id} style={{ borderBottom: i < reportedCaptions.length - 1 ? "1px solid var(--border-subtle)" : "none" }}>
                  <td style={{ ...tdStyle, maxWidth: 300 }}>
                    <span style={{ fontSize: 13, color: "var(--text-primary)", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                  ...
                  <td style={tdStyle}>
                    <Link href={`/captions?search=${r.caption_id}`} style={{ padding: "5px 12px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-subtle)", color: "var(--text-primary)", fontSize: 12, fontWeight: 600, textDecoration: "none" }}>
                      View Caption
                    </Link>
                  </td>
                </tr>
      ...
      {/* Reported Images */}
      <h2 style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)", margin: "0 0 12px" }}>
        Reported Images <span style={{ fontSize: 13, fontWeight: 400, color: "var(--text-muted)" }}>({reportedImages.length})</span>
      </h2>
      <div style={card}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {reportedImages.map((r: any, i: number) => (
                <tr key={r.id} style={{ borderBottom: i < reportedImages.length - 1 ? "1px solid var(--border-subtle)" : "none" }}>
                  <td style={tdStyle}>
                    {(r.images as any)?.url ? (
                  ...
                  <td style={tdStyle}>
                    <a href={(r.images as any)?.url ?? "/images"} target="_blank" rel="noreferrer" style={{ padding: "5px 12px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-subtle)", color: "var(--text-primary)", fontSize: 12, fontWeight: 600, textDecoration: "none" }}>
                      View Image
                    </a>
                  </td>
                </tr>
