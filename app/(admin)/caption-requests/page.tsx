  ...
  useEffect(() => {
    (async () => {
      const [{ data, error }, { count }] = await Promise.all([
        supabase.from("caption_requests").select("*").order("id", { ascending: false }).limit(200),
  ...
  return (
    <div style={{ padding: "36px 40px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 6px", letterSpacing: "-0.01em" }}>Caption Requests</h1>
          <p style={{ fontSize: 14, color: "var(--text-muted)", margin: 0 }}>
            {total !== null ? `${total.toLocaleString()} total` : `${rows.length}`} request{(total ?? rows.length) !== 1 ? "s" : ""}
              ...
              <tbody>
                {filtered.map((row, i) => (
                  <tr key={i} style={{ borderBottom: i < filtered.length - 1 ? "1px solid var(--border-subtle)" : "none" }}>
                    {cols.map((c) => (
                      <td key={c} style={{ padding: "10px 16px", fontSize: 13, color: "var(--text-primary)", maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
