...
export default function LlmModelsPage() {
  ...
  async function load() {
    setLoading(true);
    const { data, error } = await supabase.from("llm_models").select("*").order("id", { ascending: true }).limit(200);
  ...
  function openCreate() {
    ...
    if (Object.keys(form).length === 0) { form.name = ""; form.model_id = ""; form.provider_id = ""; }
  ...
  async function handleSave() {
    ...
    if (modal.mode === "create") {
      const { error } = await supabase.from("llm_models").insert([modal.form]);
      if (error) toast("Error: " + error.message);
      else { await load(); setModal(null); toast("Created!"); }
    } else {
      const { error } = await supabase.from("llm_models").update(modal.form).eq("id", modal.id);
      if (error) toast("Error: " + error.message);
  ...
  return (
    <div style={{ padding: "36px 40px" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "var(--bg-card)", borderRadius: 16, padding: 32, width: 480, maxWidth: "90vw", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 20px" }}>
              {modal.mode === "create" ? "New LLM Model" : "Edit LLM Model"}
            </h2>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 6px", letterSpacing: "-0.01em" }}>LLM Models</h1>
          <p style={{ fontSize: 14, color: "var(--text-muted)", margin: 0 }}>{rows.length} model{rows.length !== 1 ? "s" : ""}</p>
        </div>
